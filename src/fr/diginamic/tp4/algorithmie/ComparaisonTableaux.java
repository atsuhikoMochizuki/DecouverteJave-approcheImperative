...
public class ComparaisonTableaux {
        ...
        if (cleanArray1.length > cleanArray2.length) {
            ...
            for (i = 0; i < cleanArray1.length; i++) {
                for (j = 0; j < cleanArray2.length; j++) {
                    if (cleanArray1[i] == cleanArray2[j]) commonElemNbr++;
                }
            }
        } else {
            int i = 0, j = 0;
            for (i = 0; i < cleanArray2.length; i++) {
                for (j = 0; j < cleanArray1.length; j++) {
                    if (cleanArray2[i] == cleanArray1[j]) commonElemNbr++;
                }
            }
        }
        String result = "Nombre d'éléments en commun dans les deux tableaux: " + commonElemNbr;
        Utils.msgResult(result);
        return commonElemNbr;
    }

    public static int[] ArrayCleaner(int[] i_array) {
        Utils.msgInfo("Création d'un nouveau tableau exempts de doublons");
        String brutArray = "Tableau brut : " + Arrays.toString(i_array);
        Utils.msgInfo(brutArray);
        Utils.msgInfo("Nettoyage...");
        ...
        brutArray = "Nouveau tableau: " + Arrays.toString(cleanArray1);
        Utils.msgInfo(brutArray);
        return cleanArray1;
    }
    ...
    public static int removeDuplicate(int[] arrNumbers, int num) {
        ...
        for (int a = 0; a < num - 1; a++) {
            if (arrNumbers[a] != arrNumbers[a + 1]) arrTemporary[b++] = arrNumbers[a];
        }
